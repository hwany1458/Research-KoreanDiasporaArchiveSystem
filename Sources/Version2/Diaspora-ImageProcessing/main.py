...
사용법:
    ...
    python main.py --input input.jpg --output output.jpg --force-sr --force-face
...
def main():
    ...
    # 강제 실행 옵션 (조건 무시) ← 신규 추가
    parser.add_argument("--force-sr", action="store_true",
                        help="해상도 조건 무시하고 SR 강제 실행")
    parser.add_argument("--force-face", action="store_true",
                        help="얼굴 감지 실패해도 GFPGAN 강제 실행 (CNN 모델 사용)")
    parser.add_argument("--face-model", type=str, default="hog",
                        choices=["hog", "cnn"],
                        help="얼굴 감지 모델: hog(빠름) 또는 cnn(정확, GPU 권장) (기본: hog)")
    parser.add_argument("--face-min-size", type=int, default=32,
                        help="얼굴 최소 크기 px (기본: 32, 낮출수록 작은 얼굴도 감지)")
    ...
    # --force-sr: SR 조건 무시 → max_resolution을 매우 크게 설정
    if args.force_sr:
        options.sr_min_resolution = 0      # 어떤 해상도도 SR 대상
        options.sr_max_resolution = 99999  # 상한 없음
        print("  [옵션] SR 강제 실행 활성화")

    # --force-face: 얼굴 감지 건너뜀 → conditional_execution 부분 처리
    # pipeline.py에서 face_module의 should_process 결과를 무시하도록
    # face_min_size를 0으로 설정하여 항상 처리하도록 유도
    if args.force_face:
        options.face_min_size = 0
        print("  [옵션] 얼굴 향상 강제 실행 활성화")

    # --face-model cnn 선택 시 config로 전달 (image_analysis 모듈에서 사용)
    if args.face_model == "cnn":
        print("  [옵션] 얼굴 감지 모델: CNN (더 정확하지만 느림)")
